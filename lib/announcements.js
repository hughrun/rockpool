    ...
    MongoClient.connect(url, { useNewUrlParser: true}, function(err, client) {
      ...
      const db = client.db(dbName);
        const queue = function(db, callback) {
          ...
          collection.findOneAndDelete({}, {sort: {scheduled: 1}}) // sort by scheduled, ascending
          .then( doc => {
            if (doc.value) {
...
console.log(now)
if (item.tooted) {
  console.log(new Date(item.tooted.date.valueOf() + (settings.mastodon.hours_between_toots * 3.6e+6)))
}
console.log(tootDue)
console.log(needsTooting)
console.log('---')
