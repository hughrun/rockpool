      ...
      .then( res => {
        if (res) {
          ...
          .then( args => {
            ...
            let tweetDue = item.tweeted ? new Date(item.tweeted.date + (settings[env].twitter.hours_between_tweets * 3.6e+6)) < now : false
            ...
            let tootDue = item.tooted ? new Date(item.tooted.date + (settings[env].mastodon.hours_between_toots * 3.6e+6))< now : false
