...
// GET BLOGS
// include a filter so we can use the same function for all blogs, unclaimed blogs, problem blogs etc
const getBlogs = function(query) {
	return new Promise( function (resolve, reject) {
    // Use connect method to connect to the server
		MongoClient.connect(url, { useNewUrlParser: true }, function(err, client) {
		  assert.equal(null, err);
        ...
        blogs.find(query).sort({'title': 1}).toArray(function(err, docs) {
          resolve(docs)
          callback() // is this ever called?
        })
      }
      // this calls the function above and then closes the connection
      findDocuments(db, function() {
