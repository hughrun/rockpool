...
// TODO: this should be obsolute with AJAX / vuejs calls instead.
    ...
    MongoClient.connect(url, { useNewUrlParser: true }, function(err, client) {
      ...
      const db = client.db(dbName);
        const findDocuments = function(db, callback) {
          ...
          .then( docs => {
            docs.forEach( user => {
              // debug.log(user._id.toString(), typeof user._id.toString())
              // debug.log(user._id.valueOf(), typeof user._id.toString())
              user.idString = user._id.toString() // not sure why this work but valueOf() doesn't?
              // user.idString = idString
              return user
            })
            args.users = docs
            resolve(args)
