  ...
  MongoClient.connect(url, { useNewUrlParser: true }, function(err, client) {
    ...
    const findDocuments = function(db, callback) {
      ...
      .toArray(function(err, docs) {
        ...
        const allTags = docs.map(x => x = x.tags) // remove everything from the article record except the tags
...
// TODO: this should be in the "blogs" file, it's not a query but rather an action

const deleteBlogs = function(ids) { // ids is an object from the checked boxes
  // ids are strings need to be converted to ObjectIds
  const idStrings = Object.keys(ids) // the keys are the blog ids as strings and the values just say 'on'
  const array = idStrings.map(id => {
    return ObjectId(id)
  })
	return new Promise( function (resolve, reject) {
		MongoClient.connect(url, { useNewUrlParser: true }, function(err, client) {
		  assert.equal(null, err)
      const db = client.db(dbName)
      const deleteDocs = function(db, callback) {
        const blogs = db.collection('rp_blogs')
        blogs.deleteMany({_id: { $in: array}})
          .then(resolve)
      }
