    ...
    const findDocuments = function(db, callback) {
      ...
      .then(docs => {
        ...
        var topTags = sorted.slice(0, 10) // and truncate to just the top 10
          ...
          callback({tags: topTags}) // close the connection
      })
      .catch( e => reject(e) )
    }

    // this actually calls everything above
    findDocuments(db, function(args) {
      client.close().then( () => resolve(args))
    })
  })
})
...
// TODO: this should be obsolete with AJAX / vuejs calls instead.
    ...
    MongoClient.connect(url, { useNewUrlParser: true}, function(err, client) {
      ...
      const db = client.db(dbName);
        const findDocuments = function(db, callback) {
          ...
          collection.findOne({email: args.user})
          .then( user => {
            if (user && user._id) { // brand new users don't have an _id yet.
              ...
              callback({user: {email: args.user}, new: true})
            }
          })
        }
        // this actually calls everything above
