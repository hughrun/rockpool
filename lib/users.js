      ...
      const db = client.db(dbName);
        const updateUser = function(db, callback) {
          ...
          .then( doc => { // doc here is just the callback from update: no user data
            ...
            // NOTE: user here is just the original args!
            callback({user: args, new: false, error: null})
          })
          .catch( err => {
            callback({user: args, new: false, error: err})
          })
        }
        // call the function
        updateUser(db, function(args) {
    ...
    if (!blog) {
      ...
      MongoClient.connect(url, { useNewUrlParser: true }, function(err, client) {
        ...
        updateUser(db, function() {
            client.close().then( () => resolve(vals) ) // this is the vals we originally got
        })
      })
    }
  })
...
const updateUserPocketFilters = function(args) {
  return new Promise(function (resolve, reject) {
    ...
    MongoClient.connect(url, { useNewUrlParser: true }, function(err, client) {
      ...
      // call the function
      updateExclusions(db, function(args) {
        client.close().then( () => resolve(args) )
      })
    })
  })
