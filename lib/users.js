    ...
    MongoClient.connect(url, { useNewUrlParser: true }, function(err, client) {
      ...
      const db = client.db(dbName);
        const findDocuments = function(db, callback) {
          ...
          .then( user => {
            if (user) {
              user.idString = user._id.valueOf()
    ...
    MongoClient.connect(url, { useNewUrlParser: true }, function(err, client) {
      ...
      const db = client.db(dbName);
        const checkEmail = function(db, callback) {
          const collection = db.collection('rp_users') // TODO: change this to 'users' after migrating
            ...
            .then(existing => {
              if (existing) {
                if (existing._id.valueOf() == user.id) { // use valueOf() to get ObjectId as a string
                  ...
                  // NOTE: this is a small security concern since we're confirming a particular email
